...
namespace Terrain {
	...
	* @param baseLevel 高さΘとみなす高さ値(色データ0.0～1.0のどこの高さをΘとするか)
	*
	* @retval true 読み込み成功
	* @retval false 読み込み失敗
	*
	* 画像の赤素数を高さデータとみなして読み込む
	*/
	bool HeightMap::LoadFromFile(const char* path, float scale, float baseLevel)
	{
		...
		//画像データは下から上に向かって格納されていたので、上下反転しながら高さデータに変換
	...
	* 高さを調節する
	*
	* @param pos 高さをを取得する座標
	*
	* @return posの位置の高さ
	*
	* ハイトマップはXY平面に対応し、原点(0,0)からプラス方向に定義される
	*/
	...
	* 高さマップからメッシュを作成する
	*
	* @param meshBufffer メッシュ作成となるメッシュバッファ
	* @param meshName 作成するメッシュの名前
	* @param textName メッシュに張り付けるテクスチャ名
	*
	...
	*/
	bool HeightMap::CreateMesh(
		Mesh::Buffer& meshBuffer, const char* meshName, const char* textName) const
	{
		if (heights.empty()) {
		...
		for (int z = 0; z < size.y - 1; ++z) {
			for (int x = 0; x < size.x - 1; ++x) {
				const GLint a = (z + 1) * size.x + x;
				const GLint b = (z + 1) * size.x + (x + 1);
				const GLint c = z * size.x + (x + 1);
				const GLint d = z * size.x + x;
				indices.push_back(a);
				indices.push_back(b);
				indices.push_back(c);
		...
		const size_t iOffset =
			meshBuffer.AddIndexData(indices.data(), indices.size() * sizeof(GLint));
		...
		if (textName) {
			m.texture = Texture::Image2D::Create(textName);
		}
		else {
