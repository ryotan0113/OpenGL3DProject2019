...
PlayerActor::PlayerActor(const Terrain::HeightMap * hm, const Mesh::Buffer & buffer, 
	const glm::vec3 pos, const glm::vec3 & rot):SkeletalMeshActor(buffer.GetSkeletalMesh("Bikuni"), "Player", 13, pos, rot),
	heightMap(hm)
{
...
void PlayerActor::CheckRun(const GamePad& gamepad) {
	...
	if (glm::dot(move, move)) {
		...
		if (!boardingActor) {
			...
			const float frontY =
				heightMap->Height(position + move * 0.5f) - position.y - 0.01f;
